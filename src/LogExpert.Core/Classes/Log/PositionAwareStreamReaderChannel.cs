...
public class PositionAwareStreamReaderChannel : LogStreamReaderBase, ILogStreamReaderMemory
{
    ...
    private Channel<LineSegment> _lineChannel;
    private ChannelReader<LineSegment> _reader;
    private ChannelWriter<LineSegment> _writer;
    ...
    public override string ReadLine ()
    {
        ...
        try
        {
            //Channel read (more efficient than BlockingCollection)
            if (!_reader.TryRead(out segment))
            {
                // Use async path if not immediately available
                var task = _reader.ReadAsync(_cts?.Token ?? CancellationToken.None);
                segment = !task.IsCompleted
    ...
    private void RestartPipelineInternal (long startPosition)
    {
        ...
        _lineChannel = Channel.CreateBounded<LineSegment>(new BoundedChannelOptions(128)
        {
        ...
        _reader = _lineChannel.Reader;
        _writer = _lineChannel.Writer;
    ...
    private void EnqueueLine (LineSegment segment)
    {
        try
        {
            if (!_writer.TryWrite(segment))
            {
                // Use async path if buffer is full
                _writer.WriteAsync(segment, _cts.Token).AsTask().GetAwaiter().GetResult();
            }
        }
        catch (ChannelClosedException)
        {
