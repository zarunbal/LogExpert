...
namespace LogExpert
{
    ...
    public class Persister
    {
        ...
        private static void Save(string fileName, PersistenceData persistenceData)
        {
            ...
            fileElement.SetAttribute("lineCount", string.Empty + persistenceData.lineCount);
                                           ...
                                           SortedList<int, Bookmark> bookmarkList)
        {
            ...
            foreach (Bookmark bookmark in bookmarkList.Values)
            {
                ...
                bookmarkElement.SetAttribute("line", string.Empty + bookmark.LineNum);
                ...
                posXElement.InnerText = string.Empty + bookmark.OverlayOffset.Width;
                posYElement.InnerText = string.Empty + bookmark.OverlayOffset.Height;
        ...
        private static void WriteOptions(XmlDocument xmlDoc, XmlElement rootElement, PersistenceData persistenceData)
        {
            ...
            element.SetAttribute("maxDays", string.Empty + persistenceData.multiFileMaxDays);
            foreach (string fileName in persistenceData.multiFileNames)
            {
                XmlElement entryElement = xmlDoc.CreateElement("fileEntry");
                entryElement.SetAttribute("fileName", string.Empty + fileName);
            ...
            element.SetAttribute("line", string.Empty + persistenceData.currentLine);
            optionsElement.AppendChild(element);

            element = xmlDoc.CreateElement("firstDisplayedLine");
            element.SetAttribute("line", string.Empty + persistenceData.firstDisplayedLine);
            optionsElement.AppendChild(element);
            ...
            element.SetAttribute("position", string.Empty + persistenceData.filterPosition);
            optionsElement.AppendChild(element);

            element = xmlDoc.CreateElement("bookmarklist");
            element.SetAttribute("visible", persistenceData.bookmarkListVisible ? "1" : "0");
            element.SetAttribute("position", string.Empty + persistenceData.bookmarkListPosition);
            optionsElement.AppendChild(element);
                                               ...
                                               SortedList<int, RowHeightEntry> rowHeightList)
        {
            ...
            foreach (RowHeightEntry entry in rowHeightList.Values)
            {
                ...
                entryElement.SetAttribute("line", string.Empty + entry.LineNum);
                entryElement.SetAttribute("height", string.Empty + entry.Height);
