...
namespace LogExpert
{
    public class LogfileReader
    {
        ...
        private bool isFailModeCheckCallPending;
        private bool isFastFailOnGetLogLine;
        ...
        ///     thread has not returned, the method will return <code>null</code>. This is because this method is also called from
        ///     GUI thread
        ///     (e.g. LogWindow draw events). Under some circumstances, repeated calls to this method would lead the GUI to freeze.
        ///     E.g. when
        ///     trying to re-load content from disk but the file was deleted. Especially on network shares.
        /// </summary>
        /// <remarks>
        ///     Once the method detects a timeout it will enter a kind of 'fast fail mode'. That means all following calls will be
        ///     returned with
        ///     <code>null</code> immediately (without 1 second wait). A background call to GetLogLineInternal() will check if a
        ///     result is available.
        ///     If so, the 'fast fail mode' is switched off. In most cases a fail is caused by a deleted file. But it may also be
        ///     caused by slow
        ///     network connections. So all this effort is needed to prevent entering an endless 'fast fail mode' just because of
        ///     temporary problems.
        /// </remarks>
...
// this.lastReturnedLine = "";
        ...
        {
            _logger.Info("ShiftBuffers() begin for {0}{1}", this.fileName, IsMultiFile ? " (MultiFile)" : string.Empty);
            AcquireBufferListWriterLock();
            int offset = 0;
...
// this.ILogFileInfoList.Add(ILogFileInfo);
        ...
        public void stopMonitoring()
        {
            ...
            if (monitorThread != null)
            {
                ...
                {
// if thread has not finished, abort it
...
// if thread has not finished, abort it
        ...
        {
            Thread stopperThread = new Thread(stopMonitoring);
...
// _logger.logDebug("getBufferForLine(" + lineNum + ") duration: " + ((endTime - startTime)) + " ms. Buffer start line: " + logBuffer.StartLine);
        ...
        private ILogLine GetLogLineInternal(int lineNum)
        {
            ...
            {
                ReleaseBufferListReaderLock();
                _logger.Error("Cannot find buffer for line {0}, file: {1}{2}", lineNum, fileName, IsMultiFile ? " (MultiFile)" : string.Empty);
                return null;
            }
...
// this.lastReturnedLine = "";
        ...
        {
            garbageCollectorThread = new Thread(GarbageCollectorThreadProc);
