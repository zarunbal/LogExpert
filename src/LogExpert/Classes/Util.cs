...
namespace LogExpert
{
    public class Util
    {
        ...
        public static string GetFileSizeAsText(long size)
        {
            if (size < 1024)
            {
                return string.Empty + size + " bytes";
            }

            if (size < 1024 * 1024)
            {
                return string.Empty + size / 1024 + " KB";
            }

            return string.Empty + string.Format("{0:0.00}", size / 1048576.0) + " MB";
        }
        ...
        public static unsafe int YetiLevenshtein(string s1, string s2, int substitionCost)
        {
            ...
            if (xc < 0 || xc > 1)
            {
                throw new ArgumentException(string.Empty, "substitionCost");
            }
                    ...
                    if (range > 0)
                    {
                        for (int i = 0; i < range; ++i)
                        {
                            ...
                            if ((searchText.Length + 1) / (float)(dist + 1) >=
                                11F / (filterParams.fuzzyValue + 1F))
                            {
                                return true;
                            }
                        }
