...
namespace WeifenLuo.WinFormsUI.Docking
{
    public class FloatWindow : Form, INestedPanesContainer, IDockDragSource
    {
        ...
        #region Interface IDockDragSource
...
// HITTEST_CAPTION
            ...
            if (m.Msg == (int)Msgs.WM_NCRBUTTONDOWN)
            {
                uint result = NativeMethods.SendMessage(Handle, (int)Msgs.WM_NCHITTEST, 0, (uint)m.LParam);
                if (result == 2)
                {
...
// HITTEST_CAPTION
        ...
        internal void TestDrop(IDockDragSource dragSource, DockOutlineBase dockOutline)
        {
            if (VisibleNestedPanes.Count == 1)
            {
                ...
                if (NativeMethods.SendMessage(Handle, (int)Msgs.WM_NCHITTEST, 0, lParam) ==
                    (uint)HitTest.HTCAPTION)
                {
